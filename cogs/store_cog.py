...
from db.database import County, RecentPurchase, AsyncSessionLocal, async_engine

# Initialize the logger
logger = logging.getLogger(__name__)

# Define the handle_disconnect function with the correct arguments
def handle_disconnect(dbapi_connection, connection_record):
    logger.warning("Database connection disconnected.")

# Attach the handle_disconnect event listener to the database engine
...
class StoreCog(commands.Cog):
    ...
    async def load_default_counties(self, session: AsyncSession) -> None:
        ...
        try:
            ...
            logger.error(f"Error committing county data: {e}")
            await session.rollback()

    async def save_recent_purchase(self, username: str, county_name: str, price: int) -> None:
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                logger.error(f"Error saving recent purchase: {e}")
                await session.rollback()
    ...
    def create_shop_embed(self, counties_sorted: List[Tuple[str, int]]) -> discord.Embed:
        ...
        embed = discord.Embed(title="County Folders Shop", color=discord.Color.blue())
...
class ConfirmationView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await self.cog.save_recent_purchase(self.user.name, self.county_name, self.price)
            await interaction.response.send_message(
            ...
            await interaction.response.send_message("An error occurred while confirming your purchase. Please try again later.", ephemeral=True)
        finally:
